...
typedef struct {
	...
	Tcadena nombre;
	Tcadena domicilio;
	int variedadMed;  // se almacenan el total de medicamentos distintos que produce
} Tlaboratorio;
...
char menu() {
	...
	cout << "8. Información laboratorios. " << endl;
	cout << "9. Salir." << endl;
	cin >> op;
	return(op);
}
	
void MasyMenos(Tmedicamento med[], Tlaboratorio lab[], int tm, int tl, Tcadena labMax, int &mas, Tcadena labMenos, int &menos) {
  int i, j, k, contar, max, min, cod;
  ...
  for (i=0; i<tl; i++) {
    ...
    for (j=0; j<tm; j++) {
    	k = 0;
    	while (med[j].fabricante[k] != cod && k < med[j].totalFabric)
    		k++;
    	if (med[j].fabricante[k] == cod)
    		contar++;
    }
    lab[i].variedadMed = contar;
    if (i==0) {
    	mas = contar;
    	menos = contar;
    } else { 	
        if (contar > lab[max].variedadMed){
            max = i;
            mas = contar;
        }
        if (contar < lab[min].variedadMed){
            min = i;
            menos = contar;
        }   	
    }  
  }	 
  strcpy(labMax, lab[max].nombre);
  strcpy(labMenos, lab[min].nombre);   		
}
	...
	do{
		...
		switch(opcion){
			...
			case '7':
				...
				break;
			case '8':
				MasyMenos(medicamentos, laboratorios, totalMed, totalLab, labMax, mas, labMenos, menos);
				cout << labMax << " con " << mas << " medicamentos " << endl;
				cout << labMenos << " con " << menos << " medicamentos " << endl;
				break;
			case '9':
		  		cout << "Chao " << endl;
				break;
