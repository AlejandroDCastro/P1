...
typedef struct{
	...
	float precioVenta;
	Tcadena componente;
	int totalFabric;   // total de laboratorios que lo fabrican
	Tfabricante fabricante;   // vector de 10 enteros
}Tmedicamento;
...
int mostrarMedicBeneficio(Tmedicamento med[], int tm){
  float beneficio;
  int i, pos;
...
int main(){
	...
	int codigo, totalMed, totalLab, pos;
	char opcion;
	totalMed = 0;
	totalLab = 0;
	...
	do{
		...
		switch(opcion){
			...
			case '7':
                                if (totalMed>0) {
                                    pos = mostrarMedicBeneficio(medicamentos, totalMed);	
                                    cout << "Medicamento con mayor beneficio: " << endl;
                                    mostrarMedicamento(medicamentos[pos]);	
                                }else
